...
const rpcConfig = new pulumi.Config("rpc");
...
const agaveVersion = rpcConfig.get("version") ?? "2.1.13-1";

// Create some keys for this rpc to use.
const rpcKey = new svmkit.KeyPair("rpc-key");
...
new svmkit.rcp.Agave(
  "rpc-server",
  {
    ...
    flags: {
      ...
      limitLedgerSize: networkName === "mainnet" ? 100000000 : 50000000,
      blockProductionMethod: "central-scheduler",
      fullSnapshotIntervalSlots: 1000,
      noWaitForVoteToStartLeader: true,
      noVoting: true,
      accountsIndexProofSize: 2048,
      accountsIndexMemoryLimit: networkName === "mainnet" ? 300000 : 100000,
      accountsIndexSkipShrink: true,
      accountsDbCachingEnabled: true,
...
// Expose information required to SSH to the rpc host.
export const nodes_name = ["rpc-instance"];
